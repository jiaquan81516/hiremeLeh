  ...
  return (
    <div>
      <p className="section-sub">
        Analyzing skills for <strong>{courseConfig?.label}</strong>. Change your course using the
        selector at the top.
      </p>
      ...
      {suggestedSkills.length > 0 && (
        <div style={{ marginBottom: '1rem' }}>
          <div style={{ fontSize: 12, color: '#aaa', marginBottom: 6 }}>
            Suggested for {courseConfig?.label}:
          </div>
        ...
        <div className="chip-list">
          {mySkills.map((s) => (
            <div className="chip" key={s}>
              {s}
              <span className="chip-x" onClick={() => removeSkill(s)}>
                ×
              </span>
            </div>
          ))}
      ...
      {mySkills.length === 0 && (
        ...
        >
          <div style={{ fontSize: 13, color: '#aaa' }}>
            Add your skills above to see your match score
          </div>
